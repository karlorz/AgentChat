#!/usr/bin/env bash
# chrome-debug-contract: v2 (AgentChat merge)
# ============================================================
# AgentChat Chrome CDP launcher — merged from:
#   • playwright-cli chrome-debug.sh (contract v2, karlorz/agent-skills)
#     — profile management (default-user clone / repo-local / dedicated)
#     — port health checks, --check-port / --explain / --dry-run / --json / --restart
#     — detached launch that survives agent tool shells (setsid / python3 os.setsid)
#   • AgentChat scripts/start-chrome-debug.sh + start-chrome-debug.py
#     — safe .env loading (lib-env.sh, no `source` — P0 RCE fix)
#     — PROXY_SERVER support (GFW environments)
#     — hardened flags: NO --remote-allow-origins=* (any local webpage could
#       hijack the logged-in profile via CDP), CAPTCHA prevention
#       (--disable-blink-features=AutomationControlled)
#
# The AgentChat skills (AgentChat-OneWeb / IndependentTasks / WebSubAgent)
# connect to whatever serves CDP on 127.0.0.1:9222 — this launcher is the
# recommended way to start that browser with a logged-in profile.
#
# Usage: ./scripts/chrome-debug.sh [--dry-run] [--print-config] [--json] [--check-port] [--explain] [--launch-and-explain] [--restart] [URL]
#
# Profile default: default-user (global clone of real Chrome, keeps AI logins).
...
# ── AgentChat merge: safe .env loading (P0: never `source` .env) ────────────
# Loads ~/.env then <repo>/.env (project overrides). Values are literal
# strings; $(...), backticks, $VAR stay inert. process env wins.
...
# ── AgentChat merge: proxy support ───────────────────────────────────────────
# PROXY_SERVER from .env (or env). Empty/unset = no proxy (direct connection).
...
build_chrome_args() {
  ...
  # ── AgentChat merge: proxy (GFW environments) ─────────────────────────────
  if [[ -n "${PROXY_SERVER}" ]]; then
    CHROME_ARGS+=("--proxy-server=${PROXY_SERVER}")
  fi

  # ── AgentChat merge: hardened flags ───────────────────────────────────────
  # SECURITY: --remote-allow-origins=* deliberately NOT added — it would let
  # ANY webpage open in ANY local browser connect to ws://127.0.0.1:9222 and
  # take over this fully-logged-in profile. Playwright's Node/Python WebSocket
  # clients send no Origin header, so they are accepted without it.
  # CAPTCHA prevention: hide automation from Google's bot detection.
  CHROME_ARGS+=(
    --disable-blink-features=AutomationControlled
...
print_config() {
  ...
  if [[ "${JSON_OUTPUT}" == "1" ]]; then
    ...
    PROXY_SERVER_JSON="${PROXY_SERVER}" \
    ...
    json.dumps(
        {
            ...
            "proxyServer": os.environ["PROXY_SERVER_JSON"],
